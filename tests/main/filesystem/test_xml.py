...
from omniload.source.filesystem.format.readers import read_xml
from tests.main.filesystem._iterable_helpers import (
    FileItemStub,
    NonSeekableItem,
)
...
def test_unknown_tagname_yields_no_rows(tmp_path):
    ...
    path = _write_xml(tmp_path / "d.xml", "<data><item><v>a</v></item></data>")
...
def test_extension_and_format_hint_both_resolve(tmp_path):
    ...
    assert len(_read_with_tag(_write_xml(tmp_path / "by_ext.xml", body))) == 2
    hint_path = _write_xml(tmp_path / "feed.dat", body)
    rows = list(
...
def test_reads_multiple_files_flushes_each_remainder(tmp_path):
    ...
    _write_xml(tmp_path / "a.xml", '<d><item id="1"/><item id="2"/><item id="3"/></d>')
    _write_xml(tmp_path / "b.xml", '<d><item id="4"/><item id="5"/></d>')
    rows = list(
...
def test_tagname_hint_threads_end_to_end(tmp_path):
    ...
    path = _write_xml(tmp_path / "d.xml", "<data><item><n>1</n></item></data>")
...
def test_deeply_nested_is_bounded(tmp_path):
    ...
    path = _write_xml(tmp_path / "deep.xml", body)
    with pytest.raises(lxml_etree.XMLSyntaxError):
        _read_direct(path)
...
def test_malformed_xml_raises(tmp_path):
    ...
    path = _write_xml(tmp_path / "bad.xml", "<data><item><v>oops</vvv></item>")
    with pytest.raises(lxml_etree.XMLSyntaxError):
        _read_direct(path)
...
def test_chunks_at_boundary_and_flushes_partial(tmp_path):
    ...
    path = _write_xml(tmp_path / "five.xml", body)
...
def test_missing_decoder_raises_typed_install_hint(tmp_path, monkeypatch):
    ...
    path = _write_xml(tmp_path / "x.xml", "<data><item/></data>")
