...
from omniload.source.filesystem.format.readers import read_msgpack  # noqa: E402
...
def _write_msgpack(path, rows, **packb_kwargs):
...
def test_reads_single_msgpack_record(tmp_path):
    ...
    path = _write_msgpack(tmp_path / "one.msgpack", [{"id": 1, "name": "alice"}])
...
def test_reads_multiple_msgpack_records(tmp_path):
    ...
    path = _write_msgpack(tmp_path / "many.msgpack", docs)
    rows = _read_via_source(path)
...
def test_extension_and_format_hint_both_resolve(tmp_path):
    ...
    docs = [{"id": 1}, {"id": 2}]
    # extension detection
    ext_path = _write_msgpack(tmp_path / "by_ext.msgpack", docs)
    assert len(_read_via_source(ext_path)) == 2
    # explicit #msgpack hint on a non-standard extension
    hint_path = _write_msgpack(tmp_path / "feed.dat", docs)
...
def test_adversarial_values_are_normalized(tmp_path):
    ...
    path = _write_msgpack(tmp_path / "adv.msgpack", [doc], datetime=True)
...
def test_custom_ext_type_is_dlt_safe(tmp_path):
    ...
    path = _write_msgpack(tmp_path / "ext.msgpack", [{"x": msgpack.ExtType(7, b"raw")}])
    row = _read_via_source(path)[0]
...
def test_reads_multiple_files_flushes_each_remainder(tmp_path):
    ...
    _write_msgpack(tmp_path / "a.msgpack", [{"id": 1}, {"id": 2}, {"id": 3}])
    _write_msgpack(tmp_path / "b.msgpack", [{"id": 4}, {"id": 5}])
...
def test_chunks_at_boundary_and_flushes_partial(tmp_path):
    ...
    path = _write_msgpack(tmp_path / "five.msgpack", [{"id": i} for i in range(5)])
...
def test_exact_chunksize_multiple_yields_no_empty_trailing_chunk(tmp_path):
    ...
    path = _write_msgpack(tmp_path / "four.msgpack", [{"id": i} for i in range(4)])
    chunks = list(read_msgpack(iter([FileItemStub(path)]), chunksize=2))  # ty: ignore[invalid-argument-type]
...
def test_missing_decoder_raises_typed_install_hint(tmp_path, monkeypatch):
    ...
    path = _write_msgpack(tmp_path / "x.msgpack", [{"id": 1}])
...
def test_import_path_keeps_decoder_out_of_sys_modules():
    ...
    code = (
        ...
        "import omniload.source.filesystem.adapter\n"
...
def test_msgpack_loads_into_duckdb_via_parquet_loader(tmp_path):
    ...
    src = _write_msgpack(tmp_path / "docs.msgpack", [doc])
