...
def test_reads_single_bson_document(tmp_path):
    path = write_bson(tmp_path / "one.bson", [{"_id": ObjectId(OID), "name": "alice"}])
...
def test_reads_multiple_bson_documents(tmp_path):
    ...
    path = write_bson(tmp_path / "many.bson", docs)
    rows = _read_via_source(path)
...
def test_reader_yields_chunks_across_boundary(tmp_path):
    ...
    path = write_bson(tmp_path / "many.bson", [{"_id": i} for i in range(5)])
...
def test_reader_normalizes_binary_and_nested(tmp_path):
    ...
    path = write_bson(tmp_path / "rich.bson", [doc])
