...
def get_internal_date(client: imaplib.IMAP4_SSL, message_uid: str) -> Optional[Any]:
    ...
    if timestruct is None:
        raise ValueError("timestruct is None")
...
def get_message_with_internal_date(
    ...
    # FIXME: This was referred to as `data[1]` before, which seems to be wrong?
    timestruct = imaplib.Internaldate2tuple(data[0][0])
    if timestruct is None:
        raise ValueError("timestruct is None")
...
def extract_attachments(
    message: Message, filter_by_mime_type: Optional[Sequence[str]] = None
    ...
    for part in message.walk():
        ...
        file_md = ImapFileItem(  # type: ignore
            ...
            file_content=part.get_payload(decode=True),  # ty: ignore[invalid-argument-type]
        )
    ...
    if msg.is_multipart():
        for part in msg.walk():
            ...
            if content_type == "text/plain":
                body += part.get_payload(decode=True).decode(errors="ignore")  # ty: ignore[unresolved-attribute]
    else:
        body = msg.get_payload(decode=True).decode(errors="ignore")  # ty: ignore[unresolved-attribute]
