...
- `--incremental-strategy TEXT`: Defines the strategy for incremental updates. Options include `replace`, `append`, `delete+insert`, `merge`, or `scd2`. When omitted, the default is `append` for filesystem-family sources (`file://`, `s3://`, `gs://`, `az://`, `sftp://`, ...) and `replace` for ordinary sources; sources that manage their own incrementality (many SaaS/streaming sources) keep their own per-resource disposition instead of applying a run-level one. Filesystem-family sources honour an explicit `append` or `replace`, but reject the key-based strategies (`delete+insert`, `merge`, `scd2`), which need an incremental or merge key they can't supply. See [Filesystem sources](../getting-started/incremental-loading.md#filesystem-sources).
- `--filesystem-incremental`: For filesystem-family sources, filters the metadata-only file listing by modification time and appends rows only from files not recorded by the previous run. This opt-in mode is append-only. With the default temporary pipeline directory, the destination must support dlt state sync; otherwise, reuse an explicit persistent `--pipelines-dir`. Equal-boundary files are deduplicated by file URL, while an older-mtime backfill needs `--full-refresh`. Can also be set via the `FILESYSTEM_INCREMENTAL` / `OMNILOAD_FILESYSTEM_INCREMENTAL` environment variables. Defaults to `False`. See [Filesystem sources](../getting-started/incremental-loading.md#filesystem-sources).
...
- `--reshape <engine>:<spec>`: Restructures each source document before it is loaded: flattens nested objects, coerces types, and leaves arrays as lists so they normalize into child tables. `<engine>` is `python`, `jq` or `polars`. See the [Reshape transforms](../getting-started/reshape-transforms.md) documentation for the spec formats and the type-fidelity differences between engines. Can also be set via the `RESHAPE` / `OMNILOAD_RESHAPE` environment variables. Defaults to `None`.
...
- `--full-refresh`: Ignores existing pipeline state and reloads the destination table from scratch (dlt's `refresh="drop_resources"`). Useful when a source appends on every run and you want a clean reload instead of another copy, or to reset `--filesystem-incremental` before loading an older-mtime backfill. See [Filesystem sources](../getting-started/incremental-loading.md#filesystem-sources). Can also be set via the `FULL_REFRESH` / `OMNILOAD_FULL_REFRESH` environment variables. Defaults to `False`.
